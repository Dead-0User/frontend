...
const MenuTemplate = (props: TemplateProps) => {
  ...
  const handleRemoveFromCart = (item: MenuItem) => {
    // Check committed quantity
    const cartItem = cart.find((ci) => ci.menuItemId === item.id);
    const committedQty = cartItem?.committedQuantity || 0;
    ...
    if (currentQty > committedQty) {
      ...
      } else if (currentQty === 1 && committedQty === 0) {
        // Remove from cart
        if (cartItem) {
  ...
  return (
    <div className="fixed inset-0 bg-white flex justify-center">
      <div className="w-full max-w-[480px] h-full relative bg-[#B8D4D4] shadow-2xl flex flex-col overflow-hidden">
        ...
        {selectedItem && (
          <div
            className="absolute inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0"
            onClick={() => {
                ...
                {itemQuantity[selectedItem.id] > 0 ? (
                  <div className="flex items-center justify-between bg-[#F5F5F5] rounded-lg p-2">
                    <button
                      ...
                      disabled={
                        (itemQuantity[selectedItem.id] || 0) <=
                        ((cart.find((ci) => ci.menuItemId === selectedItem.id)
                          ?.committedQuantity) || 0)
                      }
                      className={`w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white transition-all active:scale-95 ${(itemQuantity[selectedItem.id] || 0) <=
                        ((cart.find((ci) => ci.menuItemId === selectedItem.id)
                          ?.committedQuantity) || 0)
        ...
        {showCart && (
          ...
          >
            <div
              className="bg-white w-full h-[90%] rounded-t-3xl overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
              style={{ animation: "slideUp 350ms ease-out" }}
            >
